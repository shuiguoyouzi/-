...
function doClear() {
    ...
    $.ajax({
        type:"post",
        url:"${pageContext.request.contextPath }/manager/cleasSession",
        data:'{"name":"null"}',
...
<body>
      ...
      <form action="<%=request.getContextPath()%>/manager/query" >
        ...
        <td>${manager.level}</td>
         <td>
             <a href="<%=request.getContextPath()%>/manager/addPage.action">添加</a>
              <a href="<%=request.getContextPath()%>/manager/editPage.action?id=${manager.manager_nb}">修改</a>
              <a href="<%=request.getContextPath()%>/manager/delete.action?id=${manager.manager_nb}">删除</a>
           </td>
       </tr>
       </c:forEach>
